...
def reduce_statevector_to_outputs(
    statevector: np.ndarray, output_qubits: list
) -> np.ndarray:
...
def test_ibmq_compiler_statevector_equivalence(nqubits, depth):
    ...
    for _ in range(5):  # repeat with different random circuits
        ...
        qiskit_reduced = reduce_statevector_to_outputs(
            qiskit_state, compiled.circ_output
        )
